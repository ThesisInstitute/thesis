    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.05fr_1fr]">
      ...
      <section className="min-w-0">
        <div className="mb-3 flex items-center justify-between gap-4">
          ...
          <span className="[font-family:var(--font-mono)] text-[0.62rem] uppercase tracking-[0.1em] text-[var(--theme-text-dim)]">
            {isRecorded ? "saved report" : "static prototype"}
          </span>
        </div>
        <TraceStatusBanner
          forecast={forecastCell}
        ...
        <p className="mt-3 text-[0.76rem] leading-[1.55] text-[var(--theme-text-dim)]">
          {savedForecast
            ? "This saved API result includes the forecast explanation, assumptions, and caveats. The original streamed tool activity was not archived. Viewing this report does not run a new forecast."
            : forecastCell.predictionRun
...
function TraceStatusBanner({
  ...
  const status = savedForecast
    ? {
        ...
        recorded: true,
        body: "The estimate and explanation come from the same completed run.",
      }
